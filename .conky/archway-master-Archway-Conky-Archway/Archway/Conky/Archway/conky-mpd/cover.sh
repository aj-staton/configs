...
CACHE="$HOME/.covers"
# API Key for Last.fm
APIKEY="INSERT YOUR LAST.FM KEY HERE"           #######  <<<<<< Don't forget to add your last.fm API Key    ######
...
cp $HOME/.conky/Archway/conky-mpd/nocover.png /tmp/conkyCover.png
echo "NoCover" >> $LOG
...
# Nope. Download.
        ...
        curl $(sed -n 's|<image size="large">\(.*\)<\/image>|\1|p' /tmp/lastfm.xml) -o "$COVER" &> /dev/null
        echo "Exit: $?" >> $LOG
...
convert /tmp/cover.png $HOME/.conky/Archway/conky-mpd/case.png -composite /tmp/cover.png >> $LOG
# Resize for immediate use
convert /tmp/cover.png -resize 160 /tmp/conkyCover.png >> $LOG
# Set current artist
